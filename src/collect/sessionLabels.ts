...
export const EVENT_CATALOG: Record<string, EventCatalogEntry> = {
  ...
  [ANALYTICS_EVENT_NAMES.pageViewed]: {
    ...
    description:
      'A página apareceu na tela (marcador `data-book-page` cruzou o gatilho de scroll). Uma vez por página por sessão (cobertura).',
    active: true,
  },
  ...
  [ANALYTICS_EVENT_NAMES.imageViewed]: {
    ...
    description:
      'Imagem entrou na viewport (exposição na tela). Não representa atenção — apenas exposição na viewport. Para interação intencional, ver image_zoomed.',
    active: true,
  },
  ...
  [ANALYTICS_EVENT_NAMES.resourceEngagementRecorded]: {
    label: 'Tempo no modal',
    description:
      'Participante fechou um modal (ODA ou Escola Digital); registra duração com o modal aberto — distinto do tempo assistido no vídeo.',
    active: true,
  },
  ...
  [ANALYTICS_EVENT_NAMES.videoCompleted]: {
    ...
    description:
      'Participante assistiu o vídeo até o final (evento ended). Exibir como “Vídeo concluído”, não como tempo no modal.',
    active: true,
  },
  [ANALYTICS_EVENT_NAMES.videoProgressRecorded]: {
    label: 'Progresso do vídeo',
    description:
      'Participante fechou o vídeo ou encerrou a sessão sem assistir até o fim; registra progresso máximo e tempo assistido.',
    active: true,
  },
  ...
  [ANALYTICS_EVENT_NAMES.sessionFinished]: {
    ...
    description:
      'Fim da sessão — tempo no livro, inatividade, abandono, profundidade de leitura e peso observado.',
    active: true,
  },
  [ANALYTICS_EVENT_NAMES.idleStarted]: {
    label: 'Inatividade iniciada',
    description:
      'Participante ficou sem interagir com a aba visível pelo tempo configurado.',
    active: true,
  },
  [ANALYTICS_EVENT_NAMES.idleFinished]: {
    label: 'Inatividade encerrada',
    description: 'Participante retomou a interação após um período inativo.',
    active: true,
  },
};
...
const METADATA_LABELS: Record<string, string> = {
  page: 'Página',
  duration_seconds: 'Duração total (s)',
  visible_time_seconds: 'Tempo visível (s)',
  ...
  reading_depth: 'Ritmo por página vista',
  ...
  abandoned_before_end: 'Abandonou antes do fim',
  abandonment_page: 'Página de abandono',
  idle_time_seconds: 'Tempo inativo (s)',
  idle_duration_seconds: 'Duração da inatividade (s)',
  ...
  watch_duration_seconds: 'Posição máxima no vídeo (s)',
  playback_wall_seconds: 'Tempo real reproduzindo (s)',
  timeline_max_seconds: 'Posição máxima no timeline (s)',
  seek_count: 'Pulos no vídeo',
  largest_skip_seconds: 'Maior pulo (s)',
  video_duration_seconds: 'Duração do arquivo (s)',
  max_progress_percent: 'Alcance no vídeo (%)',
  href_domain: 'Domínio',
...
export function getEventDisplayInfo(event: AnalyticsEvent): EventDisplayInfo {
  ...
  if (isResourceEngagementEvent(event)) {
    ...
    return {
      label: 'Tempo no modal',
      description: subtype
        ? `${subtype} — tempo com o modal aberto (não é tempo assistido no vídeo).`
