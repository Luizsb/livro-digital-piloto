...
/** Desloca o início da leitura quando a aba ficou oculta (pausa o tempo por página). */
export function shiftPageReadingStartTimes(deltaMs: number): void {
  if (deltaMs <= 0) return;
  for (const [page, viewedAt] of pageViewedAtMs) {
    pageViewedAtMs.set(page, viewedAt + deltaMs);
  }
}
