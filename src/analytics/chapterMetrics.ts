...
} from './chapterPageConfig';
import { ANALYTICS_EVENT_NAMES, BOOK_PILOT } from './eventTypes';import { loadStoredEvents } from './trackEvent';
import {
...
export function buildChapterMetrics(sessionId: string): ChapterMetrics {
  ...
  const last_page_viewed =
    pagesViewed.size > 0 ? Math.max(...pagesViewed) : null;
...
): Omit<ChapterMetrics, 'last_page_viewed' | 'completion_status'> & {
  completion_status: 'completed';
  finish_source: FinishSource;
} {
  const metrics = buildChapterMetrics(sessionId);
  return {
    pages_viewed_count: metrics.pages_viewed_count,
    pages_completed_count: metrics.pages_completed_count,
    total_pages: metrics.total_pages,
    completion_rate: metrics.completion_rate,
    reading_depth: metrics.reading_depth,
    reading_depth_label: metrics.reading_depth_label,
    completion_status: 'completed',
    finish_source: finishSource,
  };
}
